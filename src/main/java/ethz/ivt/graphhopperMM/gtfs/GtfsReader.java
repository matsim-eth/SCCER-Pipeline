...
class GtfsReader {
    ...
    GtfsReader(String id, Graph graph, PtFlagEncoder encoder, LocationIndex walkNetworkIndex) {
        ...
        this.startDate = feed.calculateStats().getCalendarDateStart();
        this.endDate = feed.calculateStats().getCalendarDateEnd();
    ...
    void readGraph() {
        LOGGER.info("buildPT");
        buildPtNetwork();
        LOGGER.info("connectStopsToStreet");
        connectStopsToStreetNetwork();
        LOGGER.info("connectStopsToStations");
    ...
    void addTrips(ZoneId zoneId, List<TripWithStopTimes> trips, int time) {
        ...
        Map<Stop, Integer> stop_ids = new HashMap<>(feed.stops.size());
        ...
        for (TripWithStopTimes trip : trips) {
            ...
            for (StopTime stopTime : trip.stopTimes) {
                ...
                if (!stop_ids.containsKey(stop)) {
                    arrivalNode = i++;
                    if (i % 1000 == 0) LOGGER.info("stop" + i);
                    ...
                    stop_ids.put(stop, arrivalNode);
                }
                else {
                    arrivalNode = stop_ids.get(stop);
                }
