...
externalities = cbind(emissions, delay = congestion$Delay)
...
hist(ext_filtered$avg_speed, main="", xlab="speed (km/h)", ylim=c(0,7000))
...
per_date_count <- ext_filtered %>% group_by(Date_trunc, Mode) %>% tally(CO2.total.)
