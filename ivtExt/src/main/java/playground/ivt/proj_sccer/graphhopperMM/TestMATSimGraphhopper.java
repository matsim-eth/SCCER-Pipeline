...
public class TestMATSimGraphhopper {
    ...
    public static void main(String[] args) {
        ...
        GraphHopperMATSim hopper = new GraphHopperMATSim(scenario.getNetwork(), new CH1903LV03PlustoWGS84());
...
// do the actual matching, get the GPX entries from a file or via stream
  ...
  //      Leg l = gpxToLeg(mapMatching, readGPX("C:\\Users\\molloyj\\Documents\\SCCER\\gps\\ch_lvs_plus_test.csv"));
   ...
   //     String result = l.getRoute().getRouteDescription();
    ...
    /**
     * The GPS data points should be tagged with the date, and leg number. With that, plans can be constructed.
     * There is also functionality to simply convert the gps files into events. @gpxToEvents.
     * @param filename
     * @return
     */
    public static List<GPXEntry> readGPX(String filename) {
        List<GPXEntry> entries = null;
        try {
            entries =  Files.lines(Paths.get(filename))
                    .skip(1)
    ...
    public static Leg createLeg(Network network, MatchResult matchResult , double departureTime, double travelTime) {
        ...
        leg.setDepartureTime(travelTime);
