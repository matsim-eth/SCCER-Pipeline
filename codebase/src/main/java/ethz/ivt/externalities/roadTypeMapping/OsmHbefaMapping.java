...
public class OsmHbefaMapping extends HbefaRoadTypeMapping {
    ...
    public String determineHebfaType(Link link) {
        ...
        hbefaType = getHEBFAtype(link,link.getFreespeed());

        return hbefaType;

    }

    private String getHEBFAtype(Link link, double freeVelocity) {
        ...
        if (hbfeaMap.get(type) == null) {
            if (link.getAllowedModes().contains(TransportMode.car)) {
                throw new RuntimeException("'" + type + "' not in hbefa map for a car link");
            } else {
                return null;
            }
        }
